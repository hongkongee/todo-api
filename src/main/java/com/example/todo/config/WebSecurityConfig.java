...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                // jwtAuthFilter를 먼저 배치해서, 이것을 통과하면 인증이 완료가 되도록 처리
                .addFilterBefore(jwtAuthFilter, UsernamePasswordAuthenticationFilter.class)
                // Exception Filter를 Auth filter 앞에 배치
...
//                                .requestMatchers(HttpMethod.POST, "/api/todos").hasRole("ADMIN")
                                // /api/auth/**은 permit이지만, /promote는 검증이 필요하기 때문에 추가 (더 앞서서 작성)
                                ...
                                .requestMatchers("/api/auth/load-profile")
                                .authenticated()
