...
@Slf4j
// RestController에서 발생하는 예외를 전역적으로 처리할 수 있게 하는 아노테이션.
// 예외 상황에 따른 응답을 REST 방식으로 클라이언트에게 전달할 수 있다.
@RestControllerAdvice
public class GlobalExceptionHandler {

    /*
    @RestControllerAdvice 로 등록된 전역 예외 처리 방식은 Controller 에서 발생된 예외만 처리합니다.
    Service, Repository, Filter 에서 발생하는 예외는 처리하지 못합니다.
    ExpiredJwtException 처럼 security filter 단에서 발생하는 예외 타입은 애초에 요청 자체가
    Controller 에 닿지 못하기 때문에 처리할 수 없습니다.
